"""Tests for trajectory graph generation."""
import sys
...
import pytest
from lib.core.trajectory import (
    path_to_state_dirs,
    namespace_node,
    build_trajectory_graph,
    build_trajectory_from_path,
    TEMPORAL_EDGE,
)
...
class TestBuildTrajectoryGraph:
    def test_single_state_namespaces_nodes(self, tmp_path):
        ...
        trajectory = build_trajectory_graph([state_dir])

        # Check namespaced nodes exist
        assert "game@0" in trajectory.nodes()
        assert "p1@0" in trajectory.nodes()
        assert "game" not in trajectory.nodes()  # Original not present

        # Check attributes preserved
        assert trajectory.nodes["game@0"]["type"] == "Game"
        assert trajectory.nodes["p1@0"]["lore"] == "5"

        # Check state_index added
        assert trajectory.nodes["game@0"]["state_index"] == "0"
    ...
    def test_single_state_namespaces_edges(self, tmp_path):
        ...
        edges = list(trajectory.edges("game@0", data=True))
        assert len(edges) == 1
        u, v, data = edges[0]
        assert u == "game@0"
        assert v == "p1@0"
    ...
    def test_two_states_adds_temporal_edges(self, tmp_path):
        ...
        trajectory = build_trajectory_graph([s0, s1])

        # Check both states present
        assert "game@0" in trajectory.nodes()
        assert "game@1" in trajectory.nodes()
        assert "p1@0" in trajectory.nodes()
        assert "p1@1" in trajectory.nodes()

        # Check temporal edges
        temporal_edges = [
            (u, v) for u, v, d in trajectory.edges(data=True)
        ...
        assert trajectory.nodes["p1@0"]["lore"] == "0"
        assert trajectory.nodes["p1@1"]["lore"] == "3"
    ...
    def test_node_disappears_no_forward_temporal_edge(self, tmp_path):
        ...
        assert "p1.card.a@0" in trajectory.nodes()
        assert "p1.card.a@1" not in trajectory.nodes()
    ...
    def test_node_appears_no_backward_temporal_edge(self, tmp_path):
        ...
        assert "p1.card.a@0" not in trajectory.nodes()
        assert "p1.card.a@1" in trajectory.nodes()
...
class TestBuildTrajectoryFromPath:
    def test_integration_with_real_example(self):
        ...
        trajectory = build_trajectory_from_path(example_path)
        ...
        temporal_count = sum(
            1 for _, _, d in trajectory.edges(data=True)
            if d.get("label") == TEMPORAL_EDGE
        )
        ...
        for i in range(5):
            assert f"game@{i}" in trajectory.nodes()
